...
public class FriendMasterImpl implements FriendMasterInterface {
	...
	public int savefriend(String firstname, String lastname, String emailid,
		...
		try {
			...
			if (users != null && users.getStatus() != null
					&& users.getStatus().booleanValue() == true) {
				result = 1;
			} else if (users != null && users.getStatus() != null
					&& users.getStatus().booleanValue() == false) {
				result = 2;
			} else {
				...
				if (emailArr != null && emailArr.length >= 2) {
					accountDetails = (Account) session
							.createCriteria(Account.class)
							.add(Restrictions.eq("name",
									emailArr[1].toLowerCase().trim())
									.ignoreCase())
				...
				if (roles != null && userrolesDetails != null) {
					roles = (Roles) session.createCriteria(Roles.class)
							.add(Restrictions.eq("id", 3)).uniqueResult();
	...
	// for save impl
	@Override
	public int savefriend(String frienduser, Boolean status,
			String friendfirstname, String friendlastname, String password,
			String friendshortname, String userName, String hashkey)
			throws Exception {
		...
		try {
			...
			users = (Users) session
					.createCriteria(Users.class)
					.add(Restrictions.eq("emailid",
							frienduser.toLowerCase().trim()).ignoreCase())
					.uniqueResult();
			...
			if (users != null) {
				...
				if (users.getUserfriendlistsForFriendid() != null
					...
					for (Userfriendlist userfriendDB : users
							.getUserfriendlistsForFriendid()) {
						if (userfriendDB != null
								&& userfriendDB.getUsersByUserid() != null
								&& userfriendDB.getUsersByUserid().getEmailid() != null) {
							if (userfriendDB.getUsersByUserid().getEmailid()
				...
				if (userfriendExist != null
						&& userfriendExist.getStatus().equals(true)) {
					result = 1;
				} else if (userfriendExist != null
				...
				// password = new sendEmail1().createAccount(frienduser,
				// friendfirstname);
				...
				session.save(users);
				hashkey = new setinfoEmail().createAccount2(frienduser,
						frienduser);
				// hashkey = new sendEmail1().createAccount(frienduser,
				// frienduser);
				System.out.println("-password--" + hashkey);
		...
		try {
			...
			if (usersTemp != null) {
				...
				if (usersTemp.getUserfriendlistsForUserid() != null
					...
					for (Userfriendlist userfriendDB : usersTemp
							.getUserfriendlistsForUserid()) {
						if (userfriendDB != null
								&& userfriendDB.getUsersByFriendid() != null
								&& userfriendDB.getUsersByFriendid()
										.getStatus() == true) {
							...
							if (userfriendDB != null
								...
								emailid=userfriendDB.getUsersByFriendid()
										.getEmailid() != null ? userfriendDB
										.getUsersByFriendid().getEmailid()
										: "";
								
								
								firstName = userfriendDB.getUsersByFriendid()
										.getFirstname() != null ? userfriendDB
										.getUsersByFriendid().getFirstname()
										: "";

								lastName = userfriendDB.getUsersByFriendid()
										.getLastname() != null ? userfriendDB
							...
							friendVo.setFirstname(userfriendDB
									.getUsersByFriendid().getFirstname());
							friendVo.setLastname(userfriendDB
									.getUsersByFriendid().getLastname());
							friendVo.setEmailid(userfriendDB
									.getUsersByFriendid().getEmailid());
							friendVo.setShortname(userfriendDB
									.getUsersByFriendid().getShortname());
			...
			crit1.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY)
					.setProjection(
							Projections.distinct(Projections.projectionList()
									.add(Projections
											.property("usersByFriendid.id"))));
			friendList = (List<Integer>) crit1.list();

			if (friendList != null && friendList.size() != 0) {
				List<Users> userList = (List<Users>) session
						.createCriteria(Users.class)
				...
				for (Users userDB : userList) {
					...
					emailid = userDB.getEmailid() != null ? userDB.getEmailid()
							: "";
					firstName = userDB.getFirstname() != null ? userDB
							.getFirstname() : "";

					lastName = userDB.getLastname() != null ? userDB
							.getLastname() : "";
