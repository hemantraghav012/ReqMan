...
public class SubscribeImpl implements SubscribeInterface
{
	...
	public int saveSubscription(RootObject rootObject, String user) throws Exception {
		...
		try {
			...
			users = (Users) session.createCriteria(Users.class).add(Restrictions.eq("emailid", user))
	...
	private void setOrganizationalPlanDetails(Userroles userrolesTemp, Users users, Customerpayment customerpayment,
		...
		Account account = (Account) session.createCriteria(Account.class)
				.add(Restrictions.eq("name", organizationName.toLowerCase()).ignoreCase()).uniqueResult();
		if (account != null && account.getId() != 0) {
			...
			sb.append("userroles");
			sb.append("set roleid=" + RequestConstants.REQUESTOR_ROLE_ID + " where id in (select ur.id from ");
			if (schemaName != null && !schemaName.trim().equals("")) {
			...
			sb.append("account as a where u.accountid=a.id and u.id=ur.userid and ");
			sb.append("a.name = '" + account.getName().trim() + "' and ur.roleid=" + RequestConstants.TEAM_MEMBER_ID
					+ " ");
			...
			for (Userroles userroles : users.getUserroleses()) {
				if (userroles != null && userroles.getRoles() != null
						&& (userroles.getRoles().getName().trim().equalsIgnoreCase(RequestConstants.ACCOUNT_ADMIN_ROLE)
								|| userroles.getRoles().getName().trim()
										.equalsIgnoreCase(RequestConstants.APP_ADMIN_ROLE)
								|| userroles.getRoles().getName().trim()
										.equalsIgnoreCase(RequestConstants.REQUESTOR_ROLE))) {
					roleTest = true;
					...
					roles = (Roles) session.createCriteria(Roles.class)
							.add(Restrictions.eq("id", RequestConstants.REQUESTOR_ROLE_ID)).uniqueResult();
	...
	private Customerpayment getCustomerpayment(RootObject rootObject) throws Exception {
		...
		if (rootObject != null && rootObject.getData() != null && rootObject.getData().getSubscription() != null) {
			...
			productName = rootObject.getData().getSubscription().getProductName() != null
					? rootObject.getData().getSubscription().getProductName() : "";

			productId = rootObject.getData().getSubscription().getProductId() != null
					? rootObject.getData().getSubscription().getProductId() : "0";

			subscriptionName = rootObject.getData().getSubscription().getName() != null
					? rootObject.getData().getSubscription().getName() : "";

			subscriptionId = rootObject.getData().getSubscription().getSubscriptionId() != null
					? rootObject.getData().getSubscription().getSubscriptionId() : "0";

			if (rootObject.getData().getSubscription().getCustomer() != null) {
				customerId = rootObject.getData().getSubscription().getCustomer().getCustomerId() != null
						? rootObject.getData().getSubscription().getCustomer().getCustomerId() : "0";
			}

			if (rootObject.getData().getSubscription().getPlan() != null) {
				planName = rootObject.getData().getSubscription().getPlan().getName() != null
						? rootObject.getData().getSubscription().getPlan().getName() : "0";
			}

			if (rootObject.getData().getSubscription().getPlan() != null) {
				planCode = rootObject.getData().getSubscription().getPlan().getPlanCode() != null
						? rootObject.getData().getSubscription().getPlan().getPlanCode() : "0";
			}
			...
			startDate = rootObject.getData().getSubscription().getCurrentTermStartsAt() != null
					? rootObject.getData().getSubscription().getCurrentTermStartsAt() : "";

			endDate = rootObject.getData().getSubscription().getCurrentTermEndsAt() != null
					? rootObject.getData().getSubscription().getCurrentTermEndsAt() : "";
